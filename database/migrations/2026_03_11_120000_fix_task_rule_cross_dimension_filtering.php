...
{
    /**
     * Bug #038: Fix task rule filtering when multiple dimensions (country, origin, type) interact.
     *
     * A rule specific to one dimension (e.g., for_origin=WO) was incorrectly excluding
     * generic rules even when the specific rule didn't match on other dimensions (e.g., for_country=EM
     * excluding rules for US matters). Each exclusion subquery now also verifies compatibility
     * with the other filter dimensions.
     *
     * Affected: event_after_insert trigger and recreate_tasks stored procedure.
     */
...
CREATE TRIGGER `event_after_insert` AFTER INSERT ON `event` FOR EACH ROW trig: BEGIN
    ...
    OPEN cur_rule;
        create_tasks: LOOP
            ...
            -- Bug #016: Child matters don't need priority deadline as they inherit from parent
...
END trig
        ...
        // Also fix the same issue in recreate_tasks stored procedure
