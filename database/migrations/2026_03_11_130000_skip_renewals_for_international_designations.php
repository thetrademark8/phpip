...
{
    /**
     * Evolution #037: Skip renewal tasks for national designations of international marks.
     *
     * TM/WO and DP/WO matters (national designations from Madrid/Hague systems)
     * do not have their own renewal dates — renewals are managed at the international
     * registration level (the WO matter). This migration:
     * 1. Updates the trigger and stored procedure to skip REN tasks for these matters
     * 2. Cleans up existing renewal tasks on TM/WO and DP/WO matters
     */
    public function up(): void
    {
        // 1. Clean up existing renewal tasks on TM/WO and DP/WO matters
        DB::statement("
            DELETE task FROM task
            JOIN event ON task.trigger_id = event.id
            JOIN matter ON event.matter_id = matter.id
            WHERE task.code = 'REN'
            AND matter.origin = 'WO'
            AND matter.category_code IN ('TM', 'DP')
        ");
...
CREATE TRIGGER `event_after_insert` AFTER INSERT ON `event` FOR EACH ROW trig: BEGIN
    ...
    DECLARE m_category CHAR(5) DEFAULT NULL;
    ...
    DECLARE cur_rule CURSOR FOR
        ...
        AND IF (task_rules.for_country IS NOT NULL,
            ...
            concat(task_rules.task, task_rules.trigger_event) NOT IN (SELECT concat(tr.task, tr.trigger_event) FROM task_rules tr WHERE (tr.for_country, tr.for_category, tr.active) = (matter.country, matter.category_code, 1)
                AND (tr.for_origin IS NULL OR tr.for_origin = matter.origin)
            )
        )
        AND IF (task_rules.for_origin IS NOT NULL,
            ...
            concat(task_rules.task, task_rules.trigger_event) NOT IN (SELECT concat(tr.task, tr.trigger_event) FROM task_rules tr WHERE (tr.for_origin, tr.for_category, tr.active) = (matter.origin, matter.category_code, 1)
                AND (tr.for_country IS NULL OR tr.for_country = matter.country)
            )
        )
    ...
    SELECT type_code, dead, expire_date, term_adjust, country, origin, category_code, parent_id INTO m_type_code, m_dead, m_expiry, m_pta, m_country, m_origin, m_category, m_parent_id FROM matter WHERE matter.id = NEW.matter_id;
    ...
    OPEN cur_rule;
        create_tasks: LOOP
            ...
            -- Skip renewal tasks for national designations of international marks (Madrid/Hague)
            -- Renewals are managed at the international registration level (WO matter)
            IF tr_task = 'REN' AND m_origin = 'WO' AND m_category IN ('TM', 'DP') THEN
                ITERATE create_tasks;
            END IF;

            -- Skip priority deadline tasks if the matter is a child (has parent_id)
            IF tr_task = 'PRID' AND m_parent_id IS NOT NULL THEN
                ITERATE create_tasks;
            END IF;
...
END trig
        ...
        // 3. Update the recreate_tasks stored procedure
...
proc: BEGIN
    ...
    SELECT e.matter_id, e.event_date, e.code, m.country, m.type_code, m.dead, m.expire_date, m.term_adjust, m.origin, m.category_code, m.parent_id INTO e_matter_id, e_event_date, e_code, m_country, m_type_code, m_dead, m_expiry, m_pta, m_origin, m_category, m_parent_id
    FROM event e
    ...
    OPEN cur_rule;
        create_tasks: LOOP
            ...
            -- Deadline for renewals that have become due in a parent case when filing a divisional
...
END proc
     ...
     *
     * Reverts the trigger and stored procedure to the previous version (bug #038 fix)
     * without the TM/WO renewal skip. Note: deleted renewal tasks cannot be restored.
     */
