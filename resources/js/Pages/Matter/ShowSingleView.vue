        ...
        <div class="lg:col-span-2 space-y-3">
          ...
          <Card>
            ...
            <CardContent class="p-3 max-h-64 overflow-y-auto">
              <div class="space-y-2">
                <div v-for="event in recentEvents" :key="event.id" class="flex items-center justify-between text-sm py-1 border-b last:border-0">
                  <div class="flex items-center gap-2">
                    <span class="font-medium">{{ translated(event.info?.name) }}</span>
                    <span v-if="event.detail" class="text-xs text-muted-foreground">({{ event.detail }})</span>
                  </div>
                  <span class="text-xs text-muted-foreground">{{ formatDate(event.event_date) }}</span>
                </div>
              </div>
            </CardContent>
          </Card>
