<template>
  <MainLayout :title="t('document.index.title')">
    <div class="space-y-4">
      <!-- Header with actions -->
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">{{ t('document.index.heading') }}</h1>
          <p class="text-muted-foreground">
            {{ t('document.index.description') }}
          </p>
        </div>
        <div class="flex gap-2">
          <Button @click="handleCreate" v-if="canWrite" size="sm">
            <Plus class="mr-2 h-4 w-4" />
            {{ t('document.index.create') }}
          </Button>
        </div>
      </div>
          ...
          <CardHeader class="pb-3">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <CardTitle class="text-base">{{ t('document.index.filters') }}</CardTitle>
              </div>
              <Button
      ...
      <div class="text-sm text-muted-foreground">
        {{ t('document.index.foundCount', template_classes.total || 0, { count: template_classes.total || 0 }) }}
      </div>
      ...
      <!-- Custom Pagination -->
      <Pagination
        :pagination="template_classes"
        @page-change="goToPage"
      />
    </div>

    <!-- Document Dialog (unified for create/view/edit) -->
    <DocumentDialog
...
onMounted(() => {
  const savedState = localStorage.getItem('document-filters-collapsed')
  if (savedState !== null) {
...
const { 
  ...
  getApiParams
} = useDocumentFilters(props.filters)
...
const tableColumns = computed(() => [
  {
    ...
    cell: ({ row }) => h('div', { 
      class: 'font-medium cursor-pointer hover:underline text-primary',
      onClick: () => showClass(row.original.id)
    }, row.original.name),
    enableSorting: true,
  },
  {
    accessorKey: 'notes',
    header: t('document.columns.notes'),
    cell: ({ row }) => h('div', { 
...
function saveFilterState(isOpen) {
  localStorage.setItem('document-filters-collapsed', String(isOpen))
}

// Handle filter updates from the DocumentFilters component
...
function refreshData() {
  ...
  router.reload({ only: ['template_classes'] })
}
</script>