<template>
  <MainLayout :title="$t('Types')">
    <div class="space-y-4">
      <!-- Header -->
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">{{ $t('Types') }}</h1>
          <p class="text-muted-foreground">{{ $t('Manage matter types') }}</p>
        </div>
        <div class="flex gap-2">
          <Button @click="createType" v-if="canWrite" size="sm">
      ...
      <div class="text-sm text-muted-foreground">
        {{ t('types.foundCount', types.total || 0, { count: types.total || 0 }) }}
      </div>
...
const columns = computed(() => [
  {
    accessorKey: 'code',
    header: t('types.fields.code'),
    meta: {
      sortable: true,
    },
    cell: ({ row }) => h('div', { 
      class: 'font-mono cursor-pointer hover:text-primary',
      onClick: () => showType(row.original.code)
    }, row.original.code),
  },
  {
    accessorKey: 'type',
    header: t('types.fields.type'),
    meta: {
      sortable: true,
    },
    cell: ({ row }) => h('div', {
