...
export function useRuleFilters(initialFilters = {}) {
  const { t } = useI18n()
  ...
  const activeFilterBadges = computed(() => {
    ...
    // Text filters
    const textFilters = {
      Task: t('rules.filters.labels.task'),
      Detail: t('rules.filters.labels.detail'),
      Trigger: t('rules.filters.labels.trigger'),
      Category: t('rules.filters.labels.category'),
      Country: t('rules.filters.labels.country'),
      Origin: t('rules.filters.labels.origin'),
      Type: t('rules.filters.labels.type')
    }
